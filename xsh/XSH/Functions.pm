...
sub xsh_init {
  ...
  if (eval { require XSH::Parser; }) {
    $_xsh=XSH::Parser->new();
    return $_xsh;
  } else {
    print STDERR "Parsing raw grammar...\n";
    require XSH::Grammar;
    return XSH::Grammar->new();
    print STDERR "... done.\n";
  }
}
...
sub help {
  my ($command)=expand @_;
  if ($command) {
    if (exists($XSH::Help::HELP{$command})) {
      print $OUT $XSH::Help::HELP{$command};
    } else {
      print $OUT "no detailed help available on $command\n";
    }
  } else {
    print $OUT $XSH::Help::HELP;
  }
}
