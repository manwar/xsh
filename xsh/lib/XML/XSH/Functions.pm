...
sub xpath_complete {
  my ($line, $word,$pos)=@_;
  my $str=toUTF8($QUERY_ENCODING,substr($line,0,$pos).$word);

  my ($xp,$local) = xpath_complete_str($str,$DEBUG);
#  __debug("COMPLETING $_[0] local $local as $xp\n");
  return () if $xp eq "";
  $xp=~/^(?:([a-zA-Z_][a-zA-Z0-9_]*):(?!:))?((?:.|\n)*)$/;
#  __debug("ID:$1 XP:$2\n");
  my ($id,$query,$doc)=_xpath([$1,$2]);

  return () unless (ref($doc));
  my $ql= eval { find_nodes([$id,$query]) };
  return () if $@;
  my %names;
  @names{ map { 
    fromUTF8($QUERY_ENCODING,
	     substr(substr($str,0,
			   length($str)
			   -length($local)).
		    $_->localname(),$pos))
